...
function scoresAverage(moviesArray) {
  ...
  return moviesArray.length !== 0
    ? parseFloat((average / moviesArray.length).toFixed(2))
    : 0;
}
...
function dramaMoviesScore(moviesArray) {
  ...
  return dramaArray.length !== 0
    ? parseFloat((average / dramaArray.length).toFixed(2))
    : 0;
}
...
function bestYearAvg(moviesArray) {
  ...
  for (let i = 0; i < copyArray.length; i++) {
    if (
      i === copyArray.length - 1 ||
      copyArray[i + 1].year != copyArray[i].year
    ) {
      let score = 0;
      if (moviesInYear === 1) {
      ...
      if (moviesInYear > 1) {
        score =
          copyArray
            .slice(i + 1 - moviesInYear, i + 1)
            .map((movie) => movie.score)
            .reduce((a, b) => a + b) / moviesInYear;
      }
      if (score > bestScore) {
